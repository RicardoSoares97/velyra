...
final class HomeViewModel: ObservableObject {
  ...
  func refresh(traktSession: TraktSession, language: String, region: String) async {
    ...
    do {
      ...
      let fallback =
        await repository.cachedFeed(language: language, region: region)
  ...
  func select(genre: GenreFilter, language: String, region: String) async {
    ...
    selectedDiscoverySection = try? await repository.loadGenre(
      genre, language: language, region: region)
  }
  ...
  func select(provider: StreamingProvider, language: String, region: String) async {
    ...
    selectedGenreID = nil
    selectedDiscoverySection = try? await repository.loadProvider(
      provider, language: language, region: region)
  }
}
