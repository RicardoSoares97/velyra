...
final class HomeFeedRepository {
  ...
  func load(language: String, region: String) async throws -> HomeFeed {
    ...
    async let seriesProvidersTask = tmdb.providers(
      kind: .series, language: language, region: region)
    ...
    let hero =
      (trendingSeries.first ?? trendingMovies.first)?.mediaItem(kind: .series) ?? .previewHero
  ...
  {
    async let moviesTask: [TMDBMediaResult] =
      if let movieGenreID = genre.movieGenreID {
    ...
    async let seriesTask: [TMDBMediaResult] =
      if let seriesGenreID = genre.seriesGenreID {
        try await tmdb.discover(
          kind: .series,
          language: language,
          region: region,
  ...
  func loadProvider(_ provider: StreamingProvider, language: String, region: String) async throws
    -> HomeSection
  {
    ...
    let combined =
      movies.map { $0.mediaItem(kind: .movie, providerName: provider.name) }
      ...
      for (index, item) in playback.prefix(20).enumerated() {
        group.addTask { [tmdb] in
          ...
          guard let tmdbID,
            ...
            let title =
              item.movie?.title ?? item.show?.title ?? String(localized: "media.unknownTitle")
            return (
              index,
              MediaItem(
  ...
  nonisolated private static func playbackSubtitle(_ item: TraktPlaybackItem) -> String? {
    guard let episode = item.episode else {
      return String(localized: "home.continueWatching.movie")
    }
    return String(
  ...
  private func curatedGenres() -> [GenreFilter] {
    [
      GenreFilter(
        id: "action", name: String(localized: "genre.action"), movieGenreID: 28,
        seriesGenreID: 10759),
      GenreFilter(
        id: "comedy", name: String(localized: "genre.comedy"), movieGenreID: 35, seriesGenreID: 35),
      GenreFilter(
        id: "drama", name: String(localized: "genre.drama"), movieGenreID: 18, seriesGenreID: 18),
      GenreFilter(
        id: "scifi", name: String(localized: "genre.scifi"), movieGenreID: 878, seriesGenreID: 10765
      ),
      GenreFilter(
        id: "thriller", name: String(localized: "genre.thriller"), movieGenreID: 53,
        seriesGenreID: 9648),
      GenreFilter(
        id: "animation", name: String(localized: "genre.animation"), movieGenreID: 16,
        seriesGenreID: 16),
      GenreFilter(
        id: "documentary", name: String(localized: "genre.documentary"), movieGenreID: 99,
        seriesGenreID: 99),
    ]
  }

  private func mergeProviders(_ movieProviders: [TMDBProvider], _ seriesProviders: [TMDBProvider])
    -> [TMDBProvider]
  {
  ...
  private func deduplicateAdjacent(_ sections: [HomeSection], excluding heroID: String)
    -> [HomeSection]
  {
  ...
  private func preferredProviders(from providers: [TMDBProvider]) -> [TMDBProvider] {
    let priorities = [
      "Netflix", "Disney Plus", "Amazon Prime Video", "Max", "Apple TV Plus", "SkyShowtime",
    ]
    var selected: [TMDBProvider] = []
    for preferred in priorities {
      if let match = providers.first(where: {
        $0.providerName.localizedCaseInsensitiveContains(preferred)
      }),
    ...
    return HomeFeed(
      ...
      genres: [
        ...
        GenreFilter(
          id: "drama", name: String(localized: "genre.drama"), movieGenreID: 18, seriesGenreID: 18),
        GenreFilter(
          id: "scifi", name: String(localized: "genre.scifi"), movieGenreID: 878,
          seriesGenreID: 10765),
        GenreFilter(
          id: "thriller", name: String(localized: "genre.thriller"), movieGenreID: 53,
          seriesGenreID: 9648),
      ...
      sections: [
        HomeSection(
          id: "preview-trending-series", title: String(localized: "home.trending.series"),
          subtitle: String(localized: "home.trending.today"), style: .poster, items: items),
        HomeSection(
          id: "preview-trending-movies", title: String(localized: "home.trending.movies"),
          subtitle: String(localized: "home.trending.today"), style: .poster,
          items: Array(items.reversed())),
        HomeSection(
          id: "preview-top-series",
          title: String(format: String(localized: "home.top10.series.country"), countryName),
          subtitle: String(localized: "home.top10.velyra.explanation"), style: .topTen, items: items
        ),
        HomeSection(
          id: "preview-top-movies",
          title: String(format: String(localized: "home.top10.movies.country"), countryName),
          subtitle: String(localized: "home.top10.velyra.explanation"), style: .topTen,
          items: Array(items.reversed())),
      ]
    )
  }
