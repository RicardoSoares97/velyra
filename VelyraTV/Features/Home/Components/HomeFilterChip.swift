    ...
    Button(action: action) {
      ...
      .background {
        Capsule(style: .continuous)
          .fill(
            isSelected ? VelyraTheme.primary.opacity(0.94) : .white.opacity(isFocused ? 0.18 : 0.09)
          )
      }
      .overlay {
        Capsule(style: .continuous)
          .stroke(
            isFocused ? VelyraTheme.focusRing : .white.opacity(0.10), lineWidth: isFocused ? 3 : 1)
      }
