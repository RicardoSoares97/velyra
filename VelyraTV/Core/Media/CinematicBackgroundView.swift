  ...
  var body: some View {
    ZStack {
      ...
      if shouldPlayVideo,
        let url = videoURL
      {
        LoopingVideoView(url: url, isPlaying: scenePhase == .active)
      ...
      LinearGradient(
        colors: [
          ...
          .black.opacity(
            reduceTransparency
              ? max(0.72, appState.preferences.backgroundOverlayOpacity)
              : appState.preferences.backgroundOverlayOpacity),
          .black.opacity(0.92),
        ],
        startPoint: .top,
        endPoint: .bottom
